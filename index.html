...
<head>
    ...
    nav { 
        ...
        box-sizing: border-box; z-index: 100; background: rgba(5, 5, 5, 0.9);
    ...
    #hero { 
        position: relative; padding: 200px 0 150px; 
        background-image: url('{{ site.data.site.bg_image }}'); 
        background-size: cover; background-position: center; 
        background-attachment: fixed; /* Makes image float/stay while scrolling */
        box-shadow: inset 0 0 0 2000px rgba(5, 5, 5, 0.7); 
    }
    ...
    .content-card {
        ...
        position: relative;
        z-index: 5;
        color: #000; /* Dark text for contrast on gray */
    }

    /* Unified Grid for Kits & Services */
    .kits-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 30px; max-width: 1100px; margin: 0 auto; }
    .kit-card { background: #111; border: 1px solid #222; border-radius: 8px; overflow: hidden; transition: 0.3s; }
    .kit-card img { 
        width: 100%; 
        aspect-ratio: 1 / 1; /* Forces all images to be square regardless of original size */
        object-fit: cover; 
    }
    ...
    #topBtn {
        ...
        padding: 15px; border-radius: 50%; cursor: pointer; display: none;
